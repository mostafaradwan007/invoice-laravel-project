...
document.addEventListener('DOMContentLoaded', function() {
  ...
  fileUploadArea.addEventListener('drop', e => { e.preventDefault(); fileUploadArea.classList.remove('dragover'); handleFileSelection(e.dataTransfer.files[0]); });
  fileInput.addEventListener('change', e => { if(e.target.files.length>0) handleFileSelection(e.target.files[0]); });
  removeFileBtn.addEventListener('click', () => { fileInput.value=''; selectedFile.classList.remove('show'); fileUploadArea.style.display='block'; importBtn.classList.remove('active'); });

  importForm.addEventListener('submit', () => { loadingOverlay.classList.add('show'); });
});
</script>
...
<div class="main-content">
  ...
  <div class="import-content">
    <div class="import-section">
      ...
      <form id="importForm" enctype="multipart/form-data" method="POST" action="{{ route('quotes.import') }}">
        @csrf
        ...
        <div class="import-actions">
          <a href="{{ route('quotes.index') }}" class="cancel-btn">Cancel</a>
          <button type="submit" class="import-btn" id="importBtn">Import Quotes</button>
        </div>
      </form>
    </div>
  </div>
